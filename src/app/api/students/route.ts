...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Role check: If staff/teacher, strictly restrict to assigned classes (max 2)
    if (auth.user.role === 'STAFF') {
      const teacherId = auth.user.teacher_id;
      const tRes = await db.execute({
        sql: 'SELECT assigned_class_id, assigned_class_id_2 FROM teachers WHERE id = ? OR user_id = ?',
        args: [teacherId || 0, auth.user.id || 0]
      });
      ...
      if (tInfo?.assigned_class_id_2) {
        const cId2 = Number(tInfo.assigned_class_id_2);
        if (!allowedClassIds.includes(cId2)) allowedClassIds.push(cId2);
      }

      if (allowedClassIds.length === 0) {
        return NextResponse.json({ error: 'Access denied: No assigned class found for this faculty member' }, { status: 403 });
      }
      ...
      // If staff specified a sectionId, verify that section belongs to an authorized class
      if (sectionId && sectionId !== 'All') {
        const secCheck = await db.execute({
          sql: 'SELECT class_id FROM sections WHERE id = ?',
          args: [Number(sectionId)]
        });
        if (secCheck.rows.length === 0 || !allowedClassIds.includes(Number(secCheck.rows[0].class_id))) {
          return NextResponse.json({ error: 'Access denied: You are only authorized to view students in your assigned class sections' }, { status: 403 });
        }
      }

      // Lock query to permitted classes
      if (sectionId && sectionId !== 'All') {
        query += ' AND s.section_id = ?';
        args.push(Number(sectionId));
        ...
        query += ' AND s.class_id IN (' + allowedClassIds.map(() => '?').join(',') + ')';
